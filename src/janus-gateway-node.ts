...
const uniq = (list: string[]): boolean =>
  list.length === [...new Set(list)].length;
...
export class Janus {
  ...
  constructor(options?: JanusOptions) {
    ...
    // this.dockerJanusImage =
    //   'canyan/janus-gateway:master_674367adb40315229b4f0ab45b7adf33cbe45fda'
  ...
  generateInstances = async (): Promise<JanusInstanceOptions[]> => {
    ...
    for (let i = 0; i < this.instancesAmount; i++) {
      instances.push({
        id: uuidv1(),
        admin_key: uuidv1(),
        server_name: `janus_instance_${i}`,
        log_prefix: `janus_instance_${i}:`,
  ...
  public initialize = async (): Promise<void> => {
    ...
    this.logger.info(`instances generated `, JSON.stringify(list, null, 2));
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      const instance = new JanusInstance({
        ...
        onMessage: (json) => {
          this.logger.info(`MESSAGE: ${JSON.stringify(json)}`);
  ...
  private launchContainers = async (instances) => {
    ...
    instances.forEach(async (JanusInstanceDefinition, i) => {
      await this.runContainer(
        `
docker run -i --rm \\
  --name ${JanusInstanceDefinition.server_name} \\
  -p ${udpStart}-${udpEnd}:${udpStart}-${udpEnd}/udp \\
  -p 8188:8188 \\
  -p 8088:8088 \\
  -p 7188:7188 \\
  -p 7088:7088 \\
  ...
  canyan/janus-gateway:0.10.7 /usr/local/bin/janus -F /usr/local/etc/janus --rtp-port-range=${udpStart}-${udpEnd} ${
    ...
    this.logger.info(`Launching Cadvisor on ${config.osPlatform}`);
...
docker run -i --rm \\
  ...
  -v /:/rootfs:ro \\
  -v /sys:/sys:ro \\
  ${config.isLinuxOS ? '-v /cgroup:/cgroup:ro' : ''} \\
  ${config.isLinuxOS ? '-v /var/run:/var/run:ro' : ''} \\
  ${config.isMacOS ? '-v /var/run/docker.sock:/var/run/docker.sock:ro' : ''} \\
  -v /var/lib/docker:/var/lib/docker:ro \\
  gcr.io/cadvisor/cadvisor:latest
`,
      { maxBuffer, containerName: 'cAdvisor' },
    );

    this.containersLaunched = true;
  };

  private runContainer = async (command, options) => {
    this.logger.info(`Container run... ${command}`);

    const container = exec(command, options, (error, stdout, stderr) => {
      this.logger.info(`${options.containerName} terminated`);
    ...
    container.stdout.on('data', (data) => {
      this.logger.info(`${options.containerName}: ` + data);
    });
    container.stderr.on('data', (data) => {
      this.logger.error(`${options.containerName} ERROR: ` + data);
    });
  };
    ...
    if (this.containersLaunched) {
      const command =
        process.platform === 'linux'
          ...
          : `FOR /F %A IN ('docker ps -q') DO docker rm -f %~A`;
  ...
  private synchronize = async (instance_id?: string): Promise<void> => {
    ...
    if (duplicates.length > 0) {
      const error = new Error(
        `rooms ids duplicated across instances - ${JSON.stringify(duplicates)}`,
      );
      this.onError(error);
    }
  ...
  private onTimeout = (user_id: string, detach: boolean) => {
    ...
    if (detach) {
      this.detachUserHandles(user_id).then(() => {
  ...
  private onConnection = async (ws, req) => {
    ...
    if (!user_id) {
      this.logger.error(
        `new connection attempt - user id is missing! closing...`,
      );
      ws.close();
      return;
    }
  ...
  private onJanusEvent = async (instance_id: string, json): Promise<void> => {
    if (!json.sender) {
      if (json.janus !== 'ack') {
        this.logger.info(
          `[?] ${instance_id} json.sender undefined - ${JSON.stringify(json)}`,
        );
      }
      return;
    }
    ...
    if (!user_id) {
      if (!this.isLocalHandle(instance_id, handle_id)) {
        this.logger.info(
          `[${handle_id}] ${instance_id} user_id not found - ${JSON.stringify(
            json,
          )}`,
        );
      }
    ...
    if (json.janus === 'trickle') {
      ...
      json.janus === 'event' &&
      json.plugindata.data &&
      (json.plugindata.data.videoroom === 'joined' ||
        json.plugindata.data.videoroom === 'event') &&
  ...
  public createRoom = async (message: any): Promise<Response> => {
    ...
    this.logger.info(`Room created context ${JSON.stringify(context)}`);
  ...
  public destroyRoom = async (message: any): Promise<Response> => {
    ...
    this.logger.info(
      `destroyRoom roomId: ${room_id} context: ${JSON.stringify(
        context,
        null,
        2,
      )}`,
    ...
    const result = await instance
      ...
      .catch((err) => {
        this.logger.error(`Error destroyRoom ${err.message}`);
        return {
          error: err.message,
        };
      });
  ...
  public joinRoom = async (user_id, message): Promise<Response> => {
    ...
    this.logger.info(
      `[${handle_id}] ${ptype} ${user_id} is joining room ${room_id} on instance ${room.instance_id}`,
    );
  ...
  public onJoinAndConfigure = async (user_id, message): Promise<Response> => {
    ...
    this.logger.info(
      `[${handle_id}] ${ptype} ${user_id} is joining (joinandconfigure) room ${room_id} on instance ${room.instance_id}`,
    );
  ...
  public onConfigure = async (message): Promise<Response> => {
    ...
    this.logger.info(
      `[${handle_id}] ${ptype} is configuring room ${room_id} on instance ${room.instance_id}`,
    );
  ...
  public onPublish = async (message: any): Promise<Response> => {
    ...
    this.logger.info(
      `[${handle_id}] user is publishing in room ${room_id} on instance ${room.instance_id}`,
    );
  ...
  public onUnpublish = async (message: any): Promise<Response> => {
    ...
    this.logger.info(
      `[${handle_id}] user is unpublishing in room ${room_id} on instance ${room.instance_id}`,
    );
  ...
  public onHangup = async (message: any): Promise<Response> => {
    ...
    this.logger.info(
      `[${handle_id}] user is hanging up in room ${room_id} on instance ${room.instance_id}`,
    );
  ...
  public onDetach = async (message: any): Promise<Response> => {
    ...
    this.logger.info(
      `[${handle_id}] user detaching in room ${room_id} on instance ${room.instance_id}`,
    );
  ...
  public onLeave = async (message: any): Promise<Response> => {
    ...
    this.logger.info(
      `[${handle_id}] user leaving room ${room_id} on instance ${room.instance_id}`,
    );
  ...
  public onTrickle = async (message: any): Promise<Response> => {
    ...
    this.logger.info(
      `[${handle_id}] got trickle in room ${room_id} on instance ${room.instance_id}`,
    );
  ...
  public onStart = async (message: any): Promise<Response> => {
    ...
    this.logger.info(
      `[${handle_id}] start in room ${room_id} on instance ${room.instance_id}`,
    );
  ...
  private selectInstance = () => {
    ...
    console.log('Instanses ', instances);
