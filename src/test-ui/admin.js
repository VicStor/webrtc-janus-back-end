...
// paths and the like.
//
var server = 'http://localhost:7088/admin'
...
function promptAccessDetails() {
  ...
  bootbox.alert({
    message:
      ...
      "' placeholder='" +
      serverPlaceholder +
      "' autocomplete='off' id='server'></input>" +
      '</div>' +
      "<div class='input-group margin-bottom-sm'>" +
      ...
      "'placeholder='" +
      secretPlaceholder +
      "' autocomplete='off' id='secret'></input>" +
      '</div>',
    closeButton: false,
    callback: function() {
      prompting = false
      ...
      for (var k in json.status) {
        ...
        if (k === 'session_timeout') {
          $('#' + k).append(
            '<button id="' +
              k +
              '_button" type="button" class="btn btn-xs btn-primary">Edit session timeout value</button>'
          )
          ...
          $('#' + k).append(
            '<button id="' +
              k +
              '_button" type="button" class="btn btn-xs btn-primary">Edit log level</button>'
          )
          $('#' + k + '_button').click(function() {
            bootbox.prompt(
          ...
          $('#' + k).append(
            '<button id="' +
              k +
              '_button" type="button" class="btn btn-xs btn-primary">Edit min NACK queue</button>'
          )
          $('#' + k + '_button').click(function() {
            bootbox.prompt(
          ...
          $('#' + k).append(
            '<button id="' +
              k +
              '_button" type="button" class="btn btn-xs btn-primary">Edit no-media timer value</button>'
          )
          $('#' + k + '_button').click(function() {
            bootbox.prompt(
          ...
          $('#' + k).append(
            '<button id="' +
              k +
              '_button" type="button" class="btn btn-xs btn-primary">Edit slowlink-threshold value</button>'
          )
          $('#' + k + '_button').click(function() {
            bootbox.prompt(
          ...
          $('#' + k).append(
            '<button id="' +
              k +
              '_button" type="button" class="btn btn-xs"></button>'
          )
          $('#' + k + '_button')
...
function resetPluginRequest() {
  ...
  $('#addattr')
    .click(addPluginMessageAttribute)
    .click()
  $('#sendmsg').click(function() {
    ...
    for (var i = 0; i < num; i++) {
      ...
      if (type === 'number') {
        ...
        if (isNaN(value)) {
          bootbox.alert(
            'Invalid value in attribute #' + (i + 1) + ' (expecting a number)'
          )
          return
        }
        ...
        if (value.toLowerCase() === 'true') {
          ...
          bootbox.alert(
            'Invalid value in attribute #' + (i + 1) + ' (expecting a boolean)'
          )
          return
        }
      }
...
function addPluginMessageAttribute() {
  ...
  $('#addattr')
    ...
    .before(
      '<tr>' +
        '	<td><input type="text" id="attrname' +
        num +
        '" placeholder="Attribute name" onkeypress="return checkEnter(this, event);" style="width: 100%;" class="pm-property form-control input-sm"></td>' +
        '	<td><input type="text" id="attrvalue' +
        num +
        '" placeholder="Attribute value" onkeypress="return checkEnter(this, event);" style="width: 100%;" class="form-control input-sm"></td>' +
        '	<td>' +
        '		<select id="attrtype' +
        num +
...
function rawHandleInfo() {
  ...
  $('#handle-info').html(
    '<pre>' + JSON.stringify(handleInfo, null, 4) + '</pre>'
  )
}
...
function prettyHandleInfo() {
  ...
  $('#handle-info').html(
    '<table class="table table-striped" id="handle-info-table"></table>'
  )
  $('#options').hide()
  for (var k in handleInfo) {
    ...
    if (k === 'plugin_specific') {
      ...
      $('#handle-info').append(
        '<h4>ICE streams</h4>' + '<div id="streams"></table>'
      )
      for (var kk in v) {
        $('#streams').append(
          '<h5>Stream #' +
            (parseInt(kk) + 1) +
            '</h5>' +
            '<table class="table table-striped" id="stream' +
            kk +
            '">' +
            '</table>'
        )
          ...
          if (sk === 'ssrc') {
            ...
            for (var ssk in sv) {
              ...
              for (var cssk in ssv) {
                var cssv = ssv[cssk]
                if (
                  cssk === 'local-candidates' ||
                  cssk === 'remote-candidates'
                ) {
                  var candidates = '<ul>'
                  for (var c in cssv) candidates += '<li>' + cssv[c] + '</li>'
                  candidates += '</ul>'
      ...
      for (var index in json.data.tokens) {
        ...
        $('#auth-tokens').append(
          '<tr>' +
            ...
            '	<td><button  id="' +
            t.token +
            '" type="button" class="btn btn-xs btn-danger">Remove token</button></td>' +
            '</tr>'
        )
